...
- **Detection Fidelity tab (Dashboard)** — executive SOC pipeline view: per-source live EPS (in-memory O(1) telemetry hooked into `insert_event`/`write_alert`/ingest rejects), engine stats (rules loaded), case-outcome funnel with headline **Fidelity % = 100×TP/(TP+FP+benign)** over resolved cases (null-guarded, `undetermined` excluded, all-time scope). Endpoint `GET /dashboard/fidelity?window=60|3600|86400` — 60s = in-memory live pulse, 1h/24h = DB-backed COUNT (epoch-safe); `totals` block (events + rate + alerts per window, unit adapts). UI tabs + `#fidelity` deep-link, 1m/1h/24h filter (localStorage `ts_fid_window`), 7.5s polling paused when tab hidden. (PRs #17, #18)
