...
- **Detection Fidelity tab (Dashboard)** — executive SOC pipeline view: per-source live EPS (in-memory O(1) telemetry hooked into `insert_event`/`write_alert`/ingest rejects), engine stats (rules loaded, alerts/min), case-outcome funnel with headline **Fidelity % = 100×TP/(TP+FP+benign)** over resolved cases (null-guarded, `undetermined` excluded). Endpoint `GET /dashboard/fidelity`; UI tabs + `#fidelity` deep-link, 7.5s polling paused when tab hidden. (PR #17)
