...
- **Detection Fidelity tab (Dashboard)** — executive SOC pipeline view: **KPI strip (sources · events · rules · alerts rate · total alerts · Fidelity %)** over **3 detail panels (data sources, top rules by alerts, recent alerts)** + a slim outcomes strip. Fidelity % = `100×TP/(TP+FP+benign)` over resolved cases (null-guarded, `undetermined` excluded, all-time scope, **dimmed + `low sample` tag below 10 classified cases**). Endpoint `GET /dashboard/fidelity?window=60|3600|86400` — 60s = in-memory live pulse, 1h/24h = DB-backed COUNT (epoch-safe); `totals` + `top_rules` + `recent_alerts` (window-scoped, one JSONL scan). UI tabs + `#fidelity` deep-link, 1m/1h/24h filter (localStorage `ts_fid_window`), 7.5s polling paused when tab hidden. (PRs #17, #18, #19)

v1.7.x — **Detection Coverage dashboard (MITRE ATT&CK Navigator heatmap)**:
- **Bundled matrix** — `app/rules/data/mitre_enterprise.json` (committed) holds 14 tactics × ≥190 unique techniques from MITRE's official STIX bundle (ATT&CK v18.1 default). `scripts/fetch_mitre_matrix.py` (stdlib only) fetches + trims the bundle from the latest ATT&CK release tag on GitHub; rerun any time to refresh.
- **Endpoint** — `GET /dashboard/coverage?window=60|3600|86400` returns `matrix_version`, `generated_at`, `stats {rules_total/mapped/unmapped, techniques_covered/total, coverage_pct, tactics_covered/total, alerts_total/unmapped}`, and the full `tactics[]` matrix (each technique row = `{id, name, covered, rules, alerts}` — gap cells included). Window semantics identical to Fidelity (alert activity only); rule coverage is all-time. 503 when the matrix file is missing/unparseable, with a message that names the file path and the refresh script.
- **MITRE validator on rule create/update** — `validate_mitre()` in `app/rules/mitre.py` is called from `_validate_rule_yaml`. Both-or-neither rule, tactic must be in `_TACTICS`, technique must match `^T\d{4}(\.\d{3})?$`, and the (tactic, technique) pair must agree with the bundled matrix. Sub-technique IDs (`.NNN`) resolve against the prefix technique's tactic list (e.g. `T1059.001` checks against `T1059`'s tactics). Missing both fields on a rule is allowed (unmapped rule — counted in `rules_unmapped`, never crashes). **Data fix**: `app/rules/rules/nginx-http-404-spike.yaml` was misfiled as Discovery; corrected to **Reconnaissance** (T1595 = Active Scanning).
- **UI tab** — new `Detection Coverage` tab in `ui/dashboard.html`: **KPI strip (rules mapped · techniques covered · tactics covered · alerts · matrix version · unmapped)** + summary line + Navigator-style heatmap (tactics as columns, techniques as rows, log-scaled alert intensity). Covered cells are clickable → drill panel pulls `GET /alerts?mitre_technique=Txxxx&limit=10`. `#coverage` hash deep-link, 1m/1h/24h filter (localStorage `ts_cov_window`), 7.5s polling, paused on tab hidden.
- **Env** — `TINYSIEM_MITRE_MATRIX_PATH` (declared in `Settings` + pinned in `conftest.py`). Empty = use the repo-bundled `app/rules/data/mitre_enterprise.json`. (PRs #20+)
