...
In `app/main.py`, find the existing line `from app.config import settings` and replace it with:

```python
from app.config import parse_cors_origins, settings
```

(Do not add a second import line — `settings` is already imported once at the top of the file; this edit extends that same line.)
